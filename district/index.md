...
4. 站点安装完成，将项目中`App_Data\ueDistrict.sql`导入到本项目的数据库中[`[1]`]([a] "库中存在该表[dbo].[umbracoExtendingDistrict]，请忽略此步骤")

5. 进入CMS后台创建Data Types[`[2]`]([a] "Property editor选择Umbraco Extending District") [`[3]`]([a] "默认是二级联动，选中Three Tier则开启三级联动"), 如下图所示：
